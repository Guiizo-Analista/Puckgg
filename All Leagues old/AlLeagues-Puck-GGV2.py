# -*- coding: utf-8 -*-
"""
Created on Sun Mar  2 05:58:45 2025
    ...
    if file_name.endswith("_matches_cleaned.xlsx"):  # Only process relevant files
        ...
        for team in teams:
            ...
            def get_result(row, team):
                if pd.isna(row["Home Score"]) or pd.isna(row["Away Score"]):  # Check if any score is missing
                    return "Not Played"
            ...
            team_df["Points"] = team_df["Match Result"].map({"Win": 3, "Draw": 1, "Loss": 0, "Not Played": 0})
        ...
        # Now, create and save the classification table for the league

        # Sum points for each team
        classification_df = pd.DataFrame([(team, team_df["Points"].sum()) for team, team_df in team_dfs.items()], columns=["Team", "Points"])

        # Sort the classification by points in descending order
        classification_df = classification_df.sort_values(by="Points", ascending=False)
        ...
        print(f"Processed {file_name} and saved team data and classification to {league_output_folder}")
